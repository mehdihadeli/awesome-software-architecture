...
- [nekzabirov/IGaming-Game-Engine](https://github.com/nekzabirov/IGaming-Game-Engine) - Production iGaming/casino engine in Kotlin/Ktor with a sealed `DomainEvent` hierarchy and a RabbitMQ publisher whose routing keys are compiler-checked via exhaustive `when (event)`. Adding a new event forces the compiler to flag the missing branch. Events are published only after the DB transaction commits, so a failed write never produces phantom messages.
