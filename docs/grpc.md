...
- [nekzabirov/IGaming-Game-Engine](https://github.com/nekzabirov/IGaming-Game-Engine) - Production iGaming/casino engine in Kotlin/Ktor exposing a gRPC API (`game.v1` proto with six services for sessions, rounds, freespins, providers, games, aggregators) alongside a published Kotlin gRPC client JAR. Demonstrates gRPC running on a separate coroutine alongside Ktor HTTP, sharing the same Koin DI graph, with a `handleGrpcCall` interceptor mapping `DomainException` → gRPC status codes.